...
export function Nav({ variant = "marketing" }: NavProps) {
  ...
  return (
    <nav
      ref={navRef}
      className="fixed top-0 left-0 right-0 z-[100] mx-3 mt-3 flex items-center justify-between px-6 py-3.5 rounded-2xl transition-[background,border-color,box-shadow] duration-300 md:mx-5 md:px-8 backdrop-blur-md"
      style={{
    ...
    >
      ...
      {menuOpen && (
        <div
          className="absolute top-[calc(100%+8px)] left-0 right-0 mx-1 rounded-2xl px-6 pb-6 pt-4 backdrop-blur-md sm:hidden"
          style={{
